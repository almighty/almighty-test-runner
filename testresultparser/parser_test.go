	...
	Describe("Parsing XML file", func() {
		Context("With Maven-surefire plugin and Junit", func() {
			It("should return output struct", func() {
				...
				Expect(Parse(getFilepath("junit_failure_test.xml"), "surefire")).To(Equal(expectedResult))
			})
		})
		Context("With Maven-surefire plugin and TestNG", func() {
			It("should return output struct", func() {
				...
				Expect(Parse(getFilepath("testNG_failure_test.xml"), "surefire")).To(Equal(expectedResult))
			})
		})
		...
		Context("With Maven-failsafe plugin", func() {
			It("should return output struct", func() {
				...
				Expect(Parse(getFilepath("failsafe-summary.xml"), "failsafe")).To(Equal(expectedResult))
			})
		})
		Context("With Gradle default plugin", func() {
			It("should return output struct", func() {
				...
				Expect(Parse(getFilepath("gradle_failure_test.xml"), "default")).To(Equal(expectedResult))
			})
		})
...
func getFilepath(filename string) string {
	_, packagefilename, _, _ := runtime.Caller(0)
	targetDir := filepath.FromSlash(path.Dir(packagefilename) + "/artifacts/")
